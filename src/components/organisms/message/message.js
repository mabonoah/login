...
export const Message = ({ type = MessageTypes.Success, onClick }) => {
    ...
    const successBtnStyle = { color: Colors.Success, borderColor: Colors.Success };
    const failBtnStyle = { color: Colors.Fail, borderColor: Colors.Fail };

    return (
        <div className="message-container">
            <section className="message-section"
                style={{ background: type === MessageTypes.Success ? Colors.Success : Colors.Fail }}>
                <MaterialIcon className="icon">
                    {type === MessageTypes.Success ? 'sentiment_very_satisfied' : 'sentiment_dissatisfied'}
                </MaterialIcon>
            ...
            <ButtonTag className="message-btn" variant='outlined' onClick={onClick}
